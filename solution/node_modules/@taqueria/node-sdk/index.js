...
var execCmd = (cmd) => new Promise((resolve, reject) => {
  ...
  (0, import_child_process.exec)(`sh -c "${escapedCmd}"`, (err, stdout, stderr) => {
    const filteredStderr = filterDockerImageMessages(stderr);
    if (err) {
    ...
    switch (taqRun) {
      ...
      case "runPostInstall":
        if (schema.postInstall) {
          const schemaArg = (0, import_shell_quote.quote)([JSON.stringify({
...
0 && (module.exports = {
  ...
  updateAddressAlias,
  writeJsonFile,
  ...require("@taqueria/protocol/types")
});
