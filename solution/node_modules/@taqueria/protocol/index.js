...
// index.ts
var taqueria_protocol_exports = {};
__export(taqueria_protocol_exports, {
  Alias: () => Alias_exports,
  Command: () => Command_exports,
  Config: () => Config_exports,
  ConfigFileV2: () => ConfigFileV2_exports,
  ...
  NetworkAccountConfig: () => NetworkAccountConfig_exports,
  NetworkConfig: () => NetworkConfig_exports,
  NonEmptyString: () => NonEmptyString_exports,
  ...
  PluginDependenciesResponse: () => PluginDependenciesResponse_exports,
  PluginInfo: () => PluginInfo_exports,
  PluginJsonResponse: () => PluginJsonResponse_exports,
  PluginProxyResponse: () => PluginProxyResponse_exports,
  ...
  Provisioner: () => Provisioner_exports,
  ProvisionerID: () => ProvisionerID_exports,
  Provisions: () => Provisions_exports,
  ProxyTaskArgs: () => ProxyTaskArgs_exports,
  ProxyTemplateArgs: () => ProxyTemplateArgs_exports,
  PublicKeyHash: () => PublicKeyHash_exports,
  ...
  SingleChar: () => SingleChar_exports,
  Task: () => Task_exports,
  Template: () => Template_exports,
  Timestamp: () => Timestamp_exports,
  Tz: () => Tz_exports,
  Url: () => Url_exports,
  Verb: () => Verb_exports,
...
module.exports = __toCommonJS(taqueria_protocol_exports);
...
var internalSchema2 = commandSchema;
...
var internalSchema3 = configSchema;
...
var internalSchema4 = configFileV2Schema;
...
var internalSchema5 = contractSchema;
...
var internalSchema6 = economicalProtocolHashSchema;
...
var internalSchema7 = environmentSchema;
...
var taskToPluginMap = import_zod50.z.record(
  import_zod50.z.union([
    import_protocol.InstalledPlugin.schemas.schema,
    import_protocol.Task.schemas.schema
  ], { description: "Task/Plugin Mapping" })
);
var operationToPluginMap = import_zod50.z.record(
  import_zod50.z.union([
    import_protocol.InstalledPlugin.schemas.schema,
    import_protocol.ParsedOperation.schemas.schema
  ], { description: "Operation/Plugin Mapping" })
);
var templateToPluginMap = import_zod50.z.record(
  import_zod50.z.union([
    import_protocol.InstalledPlugin.schemas.schema,
    import_protocol.ParsedTemplate.schemas.schema
  ])
);
var rawSchema48 = import_zod50.z.object({
  build: import_zod50.z.string({ description: "cache.build" }),
  configHash: import_zod50.z.string({ description: "cache.configHash" }),
  tasks: taskToPluginMap,
  operations: operationToPluginMap,
  templates: templateToPluginMap,
  plugins: import_zod50.z.array(import_protocol.PluginInfo.schemas.schema, { description: "cache.plugins" })
...
var mapTasksToPlugins = (config, pluginInfo, i18n) => {
  ...
  return (0, import_fluture48.attemptP)(
  ...
  ).pipe((0, import_fluture48.mapRej)((rej) => rej));
};
...
var rawSchema49 = import_zod51.z.string({ description: "SHA256" }).length(64);
...
var toSHA256 = async (value) => {
  ...
  return eager2(factory4.make(hashHex));
};
var internalSchema47 = generatedSchemas3.schema;
...
var rawOpSchema = import_zod52.z.object({
  hash: rawSchema49.describe("state.op.hash"),
  time: rawSchema42.describe("state.op.time"),
  output: import_zod52.z.unknown().describe("state.op.output")
}).describe("Persistent State Operation");
var rawTaskSchema = import_zod52.z.object({
  task: schemas45.rawSchema,
  plugin: import_zod52.z.string().min(1),
  time: rawSchema42.describe("state.task.time"),
  output: import_zod52.z.unknown().describe("state.task.output")
});
var internalOpSchema = import_zod52.z.object({
  hash: schemas49.schema.describe("state.op.hash"),
  time: schemas42.schema.describe("state.op.time"),
  output: import_zod52.z.unknown().describe("state.op.output")
});
var internalTaskSchema = import_zod52.z.object({
  task: schemas45.schema,
  plugin: import_zod52.z.string().min(1),
  time: schemas42.schema.describe("state.task.time"),
  output: import_zod52.z.unknown().describe("state.op.output")
});
var rawSchema50 = import_zod52.z.object({
  operations: import_zod52.z.record(rawOpSchema),
  tasks: import_zod52.z.record(rawTaskSchema)
});
var internalSchema48 = import_zod52.z.object({
  operations: import_zod52.z.record(internalOpSchema),
  tasks: import_zod52.z.record(internalTaskSchema)
}).transform((val) => ({
...
var internalSchema49 = internalSchema46.extend({
  operations: import_zod53.z.preprocess(
    (val) => val != null ? val : [],
    import_zod53.z.array(
      schemas16.schema,
      { description: "ParsedOperations" }
    ).optional()
  ),
  templates: import_zod53.z.preprocess(
    (val) => val != null ? val : [],
    import_zod53.z.array(
      schemas41.schema
    ).optional()
...
var rawSchema51 = rawSchema47.extend({
...
var rawSchema52 = import_zod54.z.object({
  _: import_zod54.z.array(import_zod54.z.union([import_zod54.z.string().min(1), import_zod54.z.number()])),
  projectDir: schemas35.schema,
  maxConcurrency: import_zod54.z.preprocess(
    (val) => typeof val === "string" ? parseInt(val) : Number(val),
    import_zod54.z.number().int().min(1).default(10)
  ),
  debug: import_zod54.z.preprocess(
    (val) => Boolean(val),
    import_zod54.z.boolean().default(false)
  ),
  disableState: import_zod54.z.preprocess(
    (val) => Boolean(val),
    import_zod54.z.boolean().default(false)
  ),
  logPluginRequests: import_zod54.z.preprocess(
    (val) => Boolean(val),
    import_zod54.z.boolean().default(false)
  ),
  fromVsCode: import_zod54.z.preprocess(
    (val) => Boolean(val),
    import_zod54.z.boolean().default(false)
  ),
  version: import_zod54.z.preprocess(
    (val) => Boolean(val),
    import_zod54.z.boolean().optional()
  ),
  build: import_zod54.z.preprocess(
    (val) => Boolean(val),
    import_zod54.z.boolean().optional()
  ),
  help: import_zod54.z.preprocess(
    (val) => Boolean(val),
    import_zod54.z.boolean().optional()
  ),
  yes: import_zod54.z.preprocess(
    (val) => Boolean(val),
    import_zod54.z.boolean().optional()
  ),
  plugin: schemas15.schema.optional(),
  env: import_zod54.z.string().optional(),
  quickstart: import_zod54.z.string().min(1).optional(),
  setBuild: import_zod54.z.preprocess(
    (val) => String(val),
    import_zod54.z.string().min(3)
  ),
  setVersion: import_zod54.z.string().min(3),
  pluginName: import_zod54.z.string().min(1).optional()
}, { description: "Sanitizied Args" }).passthrough();
var scaffoldRawSchema = rawSchema52.extend({
  scaffoldProjectDir: import_zod54.z.string().min(1).transform((val) => val),
  scaffoldUrl: import_zod54.z.string().min(1).url().transform((val) => val),
  branch: import_zod54.z.string().min(1).optional()
});
var initRawSchema = rawSchema52.extend({
  workflow: import_zod54.z.string().refine((val) => val === "ligo" || val === "smartpy" || val === "archetype" || val === "michelson").optional()
});
var provisionRawSchema = rawSchema52.extend({
  operation: import_zod54.z.string().min(1).describe("Operation name"),
  name: import_zod54.z.string().min(1).regex(
    /^[a-z0-9]+[a-z0-9-_]$/,
...
var templateRawSchema = rawSchema52.extend({
  template: import_zod54.z.string().min(1)
}).passthrough();
var managePluginRawSchema = rawSchema52.omit({ pluginName: true }).extend({
  pluginName: import_zod54.z.string().min(1)
});
var setEnvRawSchema = rawSchema52.extend({
  defaultEnvironment: import_zod54.z.string().min(1)
});
